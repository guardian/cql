...
import {TextSuggestionOption} from "../../types.ts";
...
export const toolsSuggestionOptionResolvers: TextSuggestionOption[] = AllTeamInfo
    .sort((a, b) => {
      if(a.team < b.team) { return -1 }
      if(a.team > b.team) { return 1 }
      return 0
    })
    .map(team => new TextSuggestionOption(team.team, team.team, team.description))
...
export function getToolsByTeams(teams: Array<Team>): Array<Tool> {
  return sortByName(AllTools.filter(tool => teams.filter(_ => _ === tool.team).length > 0))
}
